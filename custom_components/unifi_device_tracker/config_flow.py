...
class UnifiDeviceTrackerConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    def async_get_options_flow(config_entry: ConfigEntry) -> OptionsFlow:
        return UnifiDeviceTrackerOptionsFlow()
...
class UnifiDeviceTrackerOptionsFlow(OptionsFlow):
    ...
    async def async_step_init(
        ...
        currently_tracked = [
            m.lower() for m in self.config_entry.options.get(CONF_TRACKED_MACS, [])
        ]
        current_away_delay = self.config_entry.options.get(CONF_AWAY_DELAY, DEFAULT_AWAY_DELAY)
        current_home_delay = self.config_entry.options.get(CONF_HOME_DELAY, DEFAULT_HOME_DELAY)
